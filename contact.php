<?php include 'include/header.php' ?>
<div id="page-content" class="page-content">
    <div class="banner">
        <div class="jumbotron jumbotron-bg text-center rounded-0" style="background-image: url('assets/img/bg-header.jpg');">
            <div class="container">
                <h1 class="pt-5">
                    Contact
                </h1>
                <p class="lead">
                    Don't Hesitate to Contact Us.
                </p>
            </div>
        </div>
    </div>

    <section class="pb-0">
        <div class="contact1 mb-5">
            <div class="container">
                <div class="row mt-3">
                    <div class="col-lg-7">
                        <div class="contact-wrapper">

                            <h1 class="title font-weight-normal mt-0 text-left">Send Us a Message</h1>
                            <form data-aos="fade-left" data-aos-duration="1200">
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            <input class="form-control" type="text" placeholder="Full Name">
                                        </div>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            <input class="form-control" type="email" placeholder="Email">
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <textarea class="form-control" rows="3" placeholder="Message"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-lg-12 text-center">
                                        <li>
                                            <a class="btn btn-lg btn-primary mb-5 href=" mailto:hrishikeshthakkar.19@gmail.com"><i class="fa fa-envelope"></i>contact us</a>
                                        </li>
                                    </div>
                                </div>
                            </form>
                        </div>
                    </div>
                    <div class="col-lg-5">
                        <div class="detail-wrapper p-5 bg-primary">
                            <h3 class="font-weight-normal mb-3 text-light">
                                Freshcery Headquarter
                            </h3>

                            <p class="text-light">
                                Simform 501<br>
                                Bopal Ambli road<br>
                                Ahmedabad 38001
                            </p>

                            <p class="text-light">
                                <i class="fas fa-phone"></i> 9408441939<br>
                                <i class="fas fa-envelope"></i> freshcery@gmail.com
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <iframe style="margin-left: 30rem;" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d1461.151854854564!2d72.49856209372149!3d23.029141316242306!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x395e84f4a59dacf7%3A0xff3e2c6ec71f55bd!2sSimform!5e0!3m2!1sen!2sin!4v1744188538434!5m2!1sen!2sin" width="1000" height="450" frameborder="0" style="border:0;" allowfullscreen></iframe>
        <br><br><br>
    </section>
</div>
<?php include 'include/footer.php' ?>